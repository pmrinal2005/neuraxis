...
const COLS = [
  {
    title: "Platform",
    links: ["Connectors", "Orchestration", "Adaptive AI", "Governance", "Observability"],
  },
  {
    title: "Resources",
    links: ["Documentation", "Changelog", "Engineering Blog", "Security", "Status"],
  },
  {
    title: "Company",
...
export default function Footer() {
  return (
    <footer
      className="relative w-full bg-black overflow-hidden pt-20 pb-10 px-6 md:px-16 lg:px-20"
      aria-label="Footer"
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="liquid-glass rounded-[1.75rem] p-8 md:p-14 flex flex-col md:flex-row md:items-center md:justify-between gap-8">
          <div>
            <h3 className="font-heading italic text-white text-4xl md:text-6xl tracking-[-2px] leading-[0.95] max-w-xl">
              Ship the next decade
              <br />
              of data automation.
            </h3>
            <p className="mt-4 max-w-md text-sm md:text-base font-body font-light text-white/80">
              Spin up a sandbox in 60 seconds. No credit card, no quota negotiation —
              just your data and a working pipeline.
            </p>
          </div>
          <a
            href="#pricing"
            className="bg-white text-black rounded-full inline-flex items-center gap-2 px-6 py-3 text-sm font-medium whitespace-nowrap self-start"
          >
            Start Free Trial
        ...
        <div className="mt-14 grid grid-cols-2 md:grid-cols-4 gap-10">
          ...
          {COLS.map((c) => (
            <div key={c.title}>
              <h4 className="text-xs uppercase tracking-[0.18em] font-body text-white/55">
                {c.title}
              </h4>
        ...
        <div className="mt-14 pt-6 border-t border-white/8 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <span className="text-xs font-body text-white/55">
            © {new Date().getFullYear()} Neuraxis Systems, Inc. All rights reserved.
          </span>
